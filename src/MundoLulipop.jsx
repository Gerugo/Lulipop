...
export default function MundoLulipop({ perfil, onVolver }) {
  ...
  return (
    ...
    >
      ...
      {modoPegatina && (
        <div style={{ position: 'fixed', top: '20px', left: '50%', transform: 'translateX(-50%)', backgroundColor: 'rgba(255, 209, 102, 0.95)', color: '#333', padding: '12px 30px', borderRadius: '30px', fontWeight: '700', fontSize: '1.2rem', zIndex: 100, boxShadow: '0 10px 25px rgba(0,0,0,0.2)', border: '4px solid white', backdropFilter: 'blur(10px)', textAlign: 'center' }}>
          ✨ ¡Toca cualquier parte para pegar tu pegatina {modoPegatina}! ✨
        </div>
      )}

      {pegatinasColocadas.map((p) => (
        <div key={p.id} onClick={(e) => eliminarPegatina(e, p.id)} title="Toca para quitar" style={{ position: 'absolute', left: `${p.x - 25}px`, top: `${p.y - 25}px`, fontSize: '50px', cursor: 'pointer', filter: 'drop-shadow(0 5px 8px rgba(0,0,0,0.3))', zIndex: 15, transition: 'transform 0.2s' }} onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.2)'} onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}>
          {p.emoji}
        </div>
      ))}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', zIndex: 10 }}>
        ...
        />
        <span style={{ display: 'none', fontSize: '180px', animation: 'flotarMascota 5s ease-in-out infinite', filter: 'drop-shadow(0 20px 30px rgba(0,0,0,0.3))' }}>🍭</span>
      </div>
